...
def move_backward_optimized(kobuki_bot, speed):
    logger.debug(f"Moving backward with speed {speed}")
    kobuki_bot.move(-speed, -speed, 0)
    time.sleep(0.1)
    ...
    while True:
        ...
        if largest_red_contour is not None and cv2.contourArea(largest_red_contour) > AREA_THRESHOLD:
            ...
            if position == 'left':
                ...
                if distance == 'far':
                    # Move backward since the object is far
                    cv2.putText(frame, "Moving Backward", (10, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    move_backward_optimized(my_kobuki, 100)
                elif distance == 'near' and not action_completed:
                    logger.info("Ball reached. Moving forward.")
                    cv2.putText(frame, "Ball Reached. Moving Forward.", (10, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                    move_forward_optimized(my_kobuki, speed=100)
        ...
        # Break the loop on 'q' key press or if action is completed
        if cv2.waitKey(1) & 0xFF == ord('q') or action_completed:
