...
class Kobuki:
    ...
    heading = 0
    global __th1
    l_encoder_offset = None
    r_encoder_offset = None
    ...
    def __init__(self):
        ...
        __th1 = threading.Thread(target=self.read_data)
        __th1.start()
    ...
    def read_data(self):
        while 1:
            if int.from_bytes(Kobuki.seri.read(2), byteorder="little") == 333:
                ...
                # if Kobuki.prev_time==None:
                #     Kobuki.prev_time = t.time()*1000
                # z_vel = self.gyro_velocity_data()["angular velocity of z: "]
                
                # cur_t = t.time()*1000
                # dt = cur_t - Kobuki.prev_time
                # Kobuki.heading+= (2*dt)
                # Kobuki.prev_time = cur_t
    ...
    def encoder_data(self):
        ...
        # if self.l_encoder_offset == None or self.r_encoder_offset == None:
        #     self.l_encoder_offset = (Kobuki.__basic_sensor[6] << 8) | Kobuki.__basic_sensor[5]
        #     self.r_encoder_offset = (Kobuki.__basic_sensor[8] << 8) | Kobuki.__basic_sensor[7]
        #     encoderdata.update({"Left_encoder": 0})
        #     encoderdata.update({"Right_encoder": 0})
        #     return    

        left_ticks = (Kobuki.__basic_sensor[6] << 8) | Kobuki.__basic_sensor[5] #- self.l_encoder_offset
        right_ticks = (Kobuki.__basic_sensor[8] << 8) | Kobuki.__basic_sensor[7]# - self.r_encoder_offset
        encoderdata.update({"Left_encoder": left_ticks})
        encoderdata.update({"Right_encoder": right_ticks})
        return encoderdata
